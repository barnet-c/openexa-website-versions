...
(function () {
  ...
  const BEATS = 6;
  const FORM = [0, 1, 2, 3, 4, 0]; // beat → formation: the story ends where it began, on the globe, now labelled with what comes next
  ...
  const GATES = ['Signal', 'Predict', 'Decide', 'Approve · <span style="color:#d66056">reject</span>', 'Execute', '<b>Record</b>'];
  ...
  const NODES = ['Signal', 'Predict', 'Decide', 'Approve', 'Execute', 'Record'];
  ...
  CLASSES.forEach((n, c) => {
    ...
    label(1, 'tag', c === 0 ? `<b>${n}</b> · in production` : n, [p[0] + off[0], p[1] + off[1], p[2]], { anchor, m: false });
  });
  ...
  label(2, 'band', '<i></i><b>Recorded</b> · hash-chained ledger', [0.62, LEDGER_Y + 0.02, 0], { m: false });
  ...
  // beat 5: the globe again — the lifecycles beyond the first, riding orbit A
  const NEXT = ['<b>Markets · live</b>', 'Trade finance', 'Securitization', 'Energy', 'Semiconductors', 'Pharma & aerospace'];
  NEXT.forEach((n, i) => { const p = onRing(rings[0], i / 6 * 6.283 + 0.5, 1.08, 0); label(5, 'node', n, p, { spin: true }); });

  const OFFSETS = [[1.0, 0.30, 0], [0.9, -0.04, 0], [0.9, -0.04, 0], [1.15, 0.0, 0], [0.85, 0.0, 0], [1.0, 0.30, 0]];
  const M_OFFSETS = [[0, 0.98, 0], [0, 0.66, 0], [0.12, 0.62, 0], [0, 0.62, 0], [0, 0.62, 0], [0, 0.72, 0]]; // narrow screens: formation sits above the copy
  const M_SCALES = [0.30, 0.40, 0.42, 0.42, 0.42, 0.34];
  const ELEV = [0, 0, 0, 1.35, 0, 0]; // camera height per beat — the stack is seen from above
  ...
  function updateUI() {
    ...
    labels.forEach(l => {
      ...
      if (narrow() && (l.beat === 0 || l.beat === 5 || !l.m)) on = false;
      let p = l.pos;
