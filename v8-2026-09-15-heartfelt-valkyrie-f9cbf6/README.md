...
├── _redirects         No custom rules; Netlify's built-in clean URLs serve the HTML pages
└── assets/
...
On a plain static preview server, open `company.html`. Netlify's built-in clean URL handling
serves the company page without a custom rewrite. Company assets and page links are
root-relative so `/company`, `/company/` and `/company.html` resolve them consistently on a
root-mounted HTTP site.

Do not add `/company/ /company 301!`: Netlify normalizes trailing slashes before matching
redirect rules, so that rule redirects `/company` to itself. `_redirects` intentionally contains
only comments. The corrected rules take effect only after redeploying the updated site;
a browser may also retain the previous permanent redirect in its cache.
