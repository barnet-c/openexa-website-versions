/* OpenEXA — story engine
   ...
   as the reader scrolls the homepage overture:
     ...
     2 the vertical execution rail (the swarm)    3 the agentic stack (eight layers)
...
(function () {
  ...
  uniform float u_time, u_mix, u_pr, u_spin, u_fade, u_scale, u_halo;
  ...
  void main() {
    ...
    float sz = (2.1 + a_rand.x * 2.6) * (0.6 + depth * 0.8) * accent * (dust < 0.5 ? 0.7 : 1.0) * (1.0 + 0.28 * moving) * pulse;
    ...
    v_alpha = u_halo > 0.5 ? al * (accent > 1.0 ? 0.20 : 0.045) * (0.6 + 0.4 * moving) : al;
    v_col = mix(kcol(ka), kcol(kb), t);
  }`;
  const FS = `
  precision mediump float;
  uniform highp float u_halo;
  varying float v_alpha; varying vec3 v_col;
  void main() {
    float d = length(gl_PointCoord - 0.5);
    float a = u_halo > 0.5 ? pow(smoothstep(0.5, 0.0, d), 2.4) * v_alpha : smoothstep(0.5, 0.18, d) * v_alpha;
  ...
  gl.useProgram(prog);
  const U = {}; ['u_proj', 'u_view', 'u_time', 'u_mix', 'u_pr', 'u_spin', 'u_fade', 'u_scale', 'u_halo', 'u_from', 'u_to', 'u_offset', 'u_rc', 'u_ru', 'u_rv', 'u_rs', 'u_lc', 'u_ld', 'u_ln', 'u_ls'].forEach(n => U[n] = gl.getUniformLocation(prog, n));
  ...
  // 1 — the gap: an ETF's price drifts around NAV; above it agents create, below it they redeem
  const GX = 0.95;                                                    // half-width of the chart
  const priceAt = (x) => -0.36 * Math.sin(2.6 * x) - 0.05 * Math.sin(7.3 * x + 1.1) + 0.02 * Math.sin(13 * x);
  const gapExtreme = (sign) => { let bx = 0, bv = -Infinity; for (let q = 0; q <= 400; q++) { const x = -GX + 2 * GX * q / 400; const v = sign * priceAt(x); if (v > bv) { bv = v; bx = x; } } return bx; };
  const X_PREM = gapExtreme(1), X_DISC = gapExtreme(-1);
  (function gap() {
    let i = 0;
    const nNav = Math.floor(N * 0.09), nPrice = Math.floor(N * 0.34), nFill = Math.floor(N * 0.12), nAgents = Math.floor(N * 0.34);
    for (let q = 0; q < nNav; q++) set(1, i++, (rnd() * 2 - 1) * GX, gauss() * 0.004, gauss() * 0.02, 1);                    // NAV — the line the price is measured against
    for (let q = 0; q < nPrice; q++) { const x = (rnd() * 2 - 1) * GX; set(1, i++, x, priceAt(x) + gauss() * 0.014, gauss() * 0.03, 0); } // the price
    for (let q = 0; q < nFill; q++) { const x = (rnd() * 2 - 1) * GX; set(1, i++, x, priceAt(x) * rnd(), gauss() * 0.03, 7); }          // the gap, shaded
    for (let q = 0; q < nAgents; q++) { const x = (rnd() * 2 - 1) * GX; set(1, i++, x, priceAt(x), rnd(), 11); }                       // agents closing the gap
  ...
  // beat 1: the gap — NAV, the premium crest, the discount trough
  label(1, 'lay', 'NAV<i></i>', [-GX - 0.03, 0, 0], { m: false });
  label(1, 'tag', 'Price above NAV · agents <b>create</b>', [X_PREM, priceAt(X_PREM) + 0.10, 0], { anchor: 'n' });
  label(1, 'tag', 'Price below NAV · agents <b>redeem</b>', [X_DISC + 0.08, priceAt(X_DISC) - 0.10, 0], { anchor: 'sw' });
  label(1, 'band', '<i></i>The gap · small, daily, across thousands of funds', [-GX, -0.82, 0], { m: false });
  // beat 2: gates, the spur, the ledger, the swarm
  GATES.forEach((g, gi) => label(2, 'lay', `${g}<i></i>`, [-GATE_W - 0.08, gateY(gi), 0]));
  label(2, 'tag', 'Rejected · leaves the lifecycle', [lines[1].c[0] + lines[1].d[0], lines[1].c[1] + lines[1].d[1] - 0.04, 0], { anchor: 'sw', m: false });
  label(2, 'band', '<i></i><b>Settled</b> · hash-chained ledger', [0.62, LEDGER_Y + 0.02, 0], { m: false });
  label(2, 'band', '<i></i>5,000 – 50,000 agents', [0.55, gateY(0) + 0.02, 0], { m: false });
  // beat 3: eight layers
  LAYERS.forEach((n, li) => label(3, 'lay', `${String(li + 1).padStart(2, '0')} · ${li === 2 ? `<b>${n}</b>` : n}<i></i>`, [-SW - 0.08, layerY(li), 0], { m: false }));
  ...
  const OFFSETS = [[1.0, 0.30, 0], [1.16, -0.22, 0], [0.9, -0.04, 0], [1.15, 0.0, 0], [0.85, 0.0, 0]];
  const M_OFFSETS = [[0, 0.98, 0], [0, 0.62, 0], [0.12, 0.62, 0], [0, 0.62, 0], [0, 0.62, 0]]; // narrow screens: formation sits above the copy
  const M_SCALES = [0.30, 0.46, 0.42, 0.42, 0.42];
  const ELEV = [0, 0, 0, 1.35, 0]; // camera height per formation — the stack is seen from above
  ...
  const layout = () => {
    ...
    return { scale: k, offset: [off[0] * k, off[1], off[2]] };
  };
  ...
  function scrollState(dt) {
    ...
    if (reduced || !settled) { bf = bfTarget; settled = true; }
