...
(function () {
  ...
  /* ---------------- market clock (NYSE regular hours, America/New_York) ---------------- */
  const ET = 'America/New_York';
  const etFmt = new Intl.DateTimeFormat('en-US', { timeZone: ET, hour12: false, weekday: 'short', hour: '2-digit', minute: '2-digit', second: '2-digit' });
  const pad = (n) => String(n).padStart(2, '0');
  const dur = (mins) => { const h = Math.floor(mins / 60), m = Math.floor(mins % 60); return h >= 24 ? `${Math.floor(h / 24)}d ${h % 24}h` : h ? `${h}h ${pad(m)}m` : `${Math.max(1, m)}m`; };
  function nyse(d) {
    const o = {}; etFmt.formatToParts(d).forEach(p => { o[p.type] = p.value; });
    const mins = (+o.hour % 24) * 60 + +o.minute + +o.second / 60, open = 9 * 60 + 30, close = 16 * 60;
    const weekday = o.weekday !== 'Sat' && o.weekday !== 'Sun';
    if (weekday && mins >= open && mins < close) return { open: true, text: `NYSE open · closes in ${dur(close - mins)}` };
    const days = weekday && mins < open ? 0 : o.weekday === 'Fri' ? 3 : o.weekday === 'Sat' ? 2 : 1;
    return { open: false, text: `NYSE closed · opens in ${dur(days * 1440 - mins + open)}` };
  }
  const clockFmt = (tz) => new Intl.DateTimeFormat('en-US', { timeZone: tz, hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
  const clocks = [];
  if (nav) {
    const cta = $('.container > .btn', nav);
    if (cta) {
      const right = document.createElement('div'); right.className = 'nav-right';
      const mkt = document.createElement('a'); mkt.className = 'mkt'; mkt.href = 'evidence.html'; mkt.title = 'Regular trading hours, Eastern Time. US market holidays not included.';
      mkt.innerHTML = '<i></i><span></span>';
      cta.before(right); right.append(mkt, cta);
      clocks.push(() => { const s = nyse(new Date()); mkt.classList.toggle('is-open', s.open); mkt.lastElementChild.textContent = s.text; });
    }
  }
  const bottom = $('.footer .bottom');
  if (bottom) {
    const span = document.createElement('span'); span.className = 'clocks';
    span.innerHTML = '<span><b>Seattle</b> <time data-tz="America/Los_Angeles"></time></span><span><b>New York</b> <time data-tz="America/New_York"></time></span><span data-mkt></span>';
    bottom.appendChild(span);
    const fmts = $$('time[data-tz]', span).map(t => ({ t, f: clockFmt(t.dataset.tz) }));
    const mk = $('[data-mkt]', span);
    clocks.push(() => { const now = new Date(); fmts.forEach(({ t, f }) => { t.textContent = f.format(now).replace(/^24/, '00'); t.dateTime = now.toISOString(); }); const s = nyse(now); mk.textContent = s.open ? 'NYSE open' : 'NYSE closed'; mk.classList.toggle('is-open', s.open); });
  }
      ...
      try {
        ...
        if (ok) {
          ...
          if (m) {
            ...
            m.href = `mailto:${CONTACT}?subject=${encodeURIComponent('Access request — ' + (data.interest || 'OpenEXA'))}&body=${body}`;
          }
